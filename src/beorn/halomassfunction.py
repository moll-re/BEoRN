...
class HaloMassFunction:
    def __init__(self, parameters: Parameters):
        data = np.loadtxt(parameters.cosmology.ps)
        self.k_lin = data[:, 0]
        self.P_lin = data[:, 1]
    ...
    def generate_HMF(self,param):
        ...
        self.HMF = self.f_ST * rhoc0 * param.cosmology.Om * np.abs(self.dlnsigmdlnR) / 3 / self.tab_M
...
#smooth-k filter
# TODO : beta has never been set!
def W_smooth_k(x, parameters: Parameters):
    return ((1+x**parameters.halo_mass_function.Beta)**-1)

def derivative_W_smooth(x, parameters: Parameters):
    Beta = parameters.halo_mass_function.Beta
    return -(Beta*x**(Beta-1))/((1+x**Beta)**2)

def W_times_W_prime_smooth(x, parameters: Parameters):
    Beta = parameters.halo_mass_function.Beta
...
def crossing_f_ST(sigm, parameters: Parameters):
    """""
    First crossing distribution
    """""
    A = parameters.halo_mass_function.A
    q = parameters.halo_mass_function.q
    delta_c = parameters.halo_mass_function.delta_c
    p = parameters.halo_mass_function.p
