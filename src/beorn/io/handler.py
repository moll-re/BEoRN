...
class Handler:
    ...
    def write_file(self, parameters: Parameters, obj: object, **kwargs) -> Path:
        ...
        self.logger.info(f"Wrote {obj} to persistence ({file_path})")
        return file_path


    def load_file(self, parameters: Parameters, type: type, **kwargs) -> object:
        """
        ...
        """
        file_name = self.get_file_name(parameters, type, **kwargs)
        file_path = self.file_root / file_name
        with (file_path).open("rb") as f:
            obj = pickle.load(f)
        
        self.logger.info(f"Loaded {obj} from persistence ({file_path})")
        return obj
