...
def sim_par(): ## used when computing and painting profiles on a grid
    par = {
        "Mh_bin_min" : 1e5,
        "Mh_bin_max" : 1e14,
        "binn" : 80,                # to define the initial halo mass at z_ini = solver.z
...
def cosmo_par():
    par = {
        ...
        's8': 0.83,
        'ns': 0.96,
        'ps': Path(importlib.util.find_spec('beorn').origin).parent / 'files' / 'PCDM_Planck.dat',  ### This is the path to the input Linear Power Spectrum
        'corr_fct' : Path(importlib.util.find_spec('beorn').origin).parent / 'files' / 'corr_fct.dat',  ### This is the path where the corresponding correlation function will be stored. You can change it to anything.
